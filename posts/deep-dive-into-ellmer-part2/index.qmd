...
In this blog post, we took a closer look at tool calling in ellmer. We traced through the source code of the `$chat()` method. Simply, it sends a question along with a tool to the assistant, the assistant responds with a tool request, the user calls the tool and then submits the tool result to the assistant, and finally the assistant responds with its final answer. More on other cool capabilities of ellmer, such as structured data, in the next blog post. 
