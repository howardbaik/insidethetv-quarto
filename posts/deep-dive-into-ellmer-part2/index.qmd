...
From here on, we run a query with the `$chat()` method using our user prompt `"How long ago did Neil Armstrong touch down on the moon?"`
```{r}
...
```
In order to unpack the iterators within a [generator](https://coro.r-lib.org/articles/generator.html#generators), 
We can collect all values of iterator with `coro::collect()`:
...
The OpenAI/gpt-4.1 model responded with streaming response data:
...
To recap, the user asked the assistant a question, "How long ago did Neil Armstrong touch down on the moon?" and the assistant responded with a tool request, asking the user to run the tool function, `get_current_time(tz = "UTC")`.
