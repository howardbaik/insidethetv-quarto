...
Note that the [tool calling vignette](https://ellmer.tidyverse.org/articles/tool-calling.html#defining-a-tool-function) introduces a helper function that uses an LLM to generate this `tool()` call, `create_tool_def()`.
...
In this section, we walk through the source code that power the `$chat()` method (as it pertains to tool calling) with the prompt `"How long ago did Neil Armstrong touch down on the moon?"`:
```{r}
...
To better understand the "whole game" of tool calling, I've created another diagram of tool calling functions in ellmer:
...
So far, we have `user_turn` (user turn) and `turn` (assistant turn):
...
This concludes the `private$submit_turns()` method. To recap, the user asked the assistant a question, "How long ago did Neil Armstrong touch down on the moon?" and the assistant responded with a tool request, asking the user to run the tool function `get_current_time(tz = "UTC")`.
...
Let's take a look at the `invoke_tool()`, which invokes each individual tool:
The `invoke_tools()` function loops through each tool request and calls the `invoke_tool()` function on each tool request. 
...
Tangentially, `ContentToolRequest` and `ContentToolResult` are S7 classes used for tool-calling in ellmer. 
...
Finally, the output of `invoke_tools()` is a generator instance (created by calling a generator function):
```{r}
> tool_calls
<generator/instance>
function(
...
We store the tool result in `user_turn` (user turn)
...
```{r}
<Turn: user>
[tool result  (call_[redacted])]: 2025-09-05 03:37:59 UTC
```

After completion of the `while` loop, we go back to the top of the loop.
...
Since `turn_has_tool_request(assistant_turn)` is `FALSE`, that completes the while loop. We go to the top of the while loop, and exit out since `user_turn` is `NULL`.

This concludes `private$chat_impl()`, and hence completes the `$chat()` method.
