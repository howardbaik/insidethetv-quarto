...
In the rest of this blog post, we will examine the tool calling functionality within the `$chat()` method of the `Chat` object. We will draw an example from the [tool calling vignette](https://ellmer.tidyverse.org/articles/tool-calling.html){target="_blank"} in ellmer.
...
Note that the [tool calling vignette](https://ellmer.tidyverse.org/articles/tool-calling.html#defining-a-tool-function){target="_blank"} introduces a helper function that uses an LLM to generate this `tool()` call, `create_tool_def()`. This function may make mistakes, so the user must review the generated code. However, for more complicated functions, `create_tool_def()` is definitely a time-saver. 
...
As shown in the diagram of ellmer's chat implementation in [Part 1](https://www.howardbaik.com/posts/deep-dive-into-ellmer-part1/#chat-implementation){target="_blank"}, the chat implementation method `$chat_impl()` leads to `$submit_turns()`, which then leads to `chat_perform()`. 
