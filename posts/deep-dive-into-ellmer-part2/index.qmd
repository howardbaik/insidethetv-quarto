...
The chat implementation method `$chat_impl()` leads to `$submit_turns()`,
...
Iterating through `response` with 
```{r}
...
Alternatively, you can collect all values of iterator with `coro::collect()` to get all values:
```{r}
...
Creates a Turn object for the assistant turn with `ContentToolRequest`, result, and tokens.

```{r}
turn <- match_tools(turn, private$tools)
```

Insert our tool (`get_current_time()`) in the `turn` object.  

So far, we have `user_turn` and `assistant_turn`:
